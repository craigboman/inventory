...
function onOpen() {
  ...
  menuEntries.push(null); // line separator
  menuEntries.push({name: "Clean Slate", functionName: "deleteAllButOneSheet"});
  menuEntries.push(null); // line separator
  ...
  for (var i=0; i<values.length; i++){
    if(values[i][1] == '') {
      ...
      if(json_data) {
        ...
        values[i][6] = '=\"' + Utilities.formatDate(new Date(), SpreadsheetApp.getActive().getSpreadsheetTimeZone(), "yyyy-MM-dd' 'HH:mm:ss") + '\"';
        values[i][7] = i+1;
...
function onEdit(e) {
  ...
  if( e.range.getValue() && (e.range.getNumRows() == 1) && sheet.getIndex() == 1) {
    ...
    e.range.offset(0,6).setValue('=\"' + Utilities.formatDate(new Date(), SpreadsheetApp.getActive().getSpreadsheetTimeZone(), "yyyy-MM-dd' 'HH:mm:ss") + '\"');
...
function deleteAllButOneSheet() {

  var sheetReturn = SpreadsheetApp.getActiveSpreadsheet().getSheets();
  Logger.log(sheetReturn);


  //loop compares array length of total sheets, deletes active sheet til sheetReturn=1   
  for(var i = 1 ; i < sheetReturn.length ;i++) { 
    Logger.log(sheetReturn[i]);
     // deletes active sheet
     SpreadsheetApp.getActiveSpreadsheet().deleteActiveSheet();
  
  } //end of for loop  
   
  //renames active sheet to inventory
  SpreadsheetApp.getActiveSpreadsheet().renameActiveSheet("Inventory");
  
  
  //flushes any changes which may be in the buffer
  ...
  //deletes all rows or columns with data
  ss.deleteColumns(1, columns);
  
} //end of deleteAllButOneSheet()
